...
function run_cmd() {
  ...
  stty -echoctl
  # Quoted so that multi-line commands (backslash continuations) keep their
  # argument boundaries. Unquoted, $@ word-splits and the escaped newline turns
  # into a leading space on the next flag. Local change; upstream has `eval $@`.
  eval "$@"
  stty echoctl
  trap - SIGINT
}
