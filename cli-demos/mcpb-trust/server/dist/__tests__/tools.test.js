...
import { registerTriggerUrlElicitationTool, __resetIssuedErrorPathElicitations, } from '../tools/trigger-url-elicitation.js';
...
describe('Tools', () => {
    ...
    describe('trigger-long-running-operation', () => {
        it('should complete operation and return result', async () => {
            ...
            const result = await handler({ duration: 0.1, steps: 2 }, { _meta: {}, requestId: 'test-123' });
        ...
        it('should send progress notifications when progressToken provided', async () => {
            ...
            registerTriggerLongRunningOperationTool(mockServer);
            const handler = handlers.get('trigger-long-running-operation');
            await handler({ duration: 0.1, steps: 2 }, { _meta: { progressToken: 'token-123' }, requestId: 'test-456', sessionId: 'session-1' });
            expect(mockServer.server.notification).toHaveBeenCalledTimes(2);
            expect(mockServer.server.notification).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe('get-resource-reference', () => {
        ...
        it('should reject invalid resource type', async () => {
            ...
            registerGetResourceReferenceTool(mockServer);
            const handler = handlers.get('get-resource-reference');
            await expect(handler({ resourceType: 'Invalid', resourceId: 1 })).rejects.toThrow('Invalid resourceType');
        });
        it('should reject invalid resource ID', async () => {
            const { mockServer, handlers } = createMockServer();
            registerGetResourceReferenceTool(mockServer);
            const handler = handlers.get('get-resource-reference');
            await expect(handler({ resourceType: 'Text', resourceId: -1 })).rejects.toThrow('Invalid resourceId');
            await expect(handler({ resourceType: 'Text', resourceId: 0 })).rejects.toThrow('Invalid resourceId');
            await expect(handler({ resourceType: 'Text', resourceId: 1.5 })).rejects.toThrow('Invalid resourceId');
        });
    });
    ...
    describe('trigger-sampling-request', () => {
        ...
        it('should register when client supports sampling', () => {
            ...
            registerTriggerSamplingRequestTool(mockServer);
            expect(mockServer.registerTool).toHaveBeenCalledWith('trigger-sampling-request', expect.objectContaining({
        ...
        it('should send sampling request and return result', async () => {
            ...
            const result = await handler({ prompt: 'Test prompt', maxTokens: 50 }, { sendRequest: mockSendRequest });
    ...
    describe('trigger-elicitation-request', () => {
        ...
        it('should register when client supports elicitation', () => {
            ...
            registerTriggerElicitationRequestTool(mockServer);
            expect(mockServer.registerTool).toHaveBeenCalledWith('trigger-elicitation-request', expect.objectContaining({
    ...
    describe('trigger-url-elicitation', () => {
        ...
        it('should register when client supports URL elicitation', () => {
            ...
            registerTriggerUrlElicitationTool(mockServer);
            expect(mockServer.registerTool).toHaveBeenCalledWith('trigger-url-elicitation', expect.objectContaining({
        ...
        it('should send URL-mode elicitation request when errorPath is false', async () => {
            ...
            expect(result.content[0].text).toContain('✅ User completed the URL elicitation flow.');
        });
        ...
        it('should default the elicitationId to a random UUID when omitted', async () => {
            ...
            expect(sentParams.elicitationId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
        });
        ...
        it('should report a cancelled URL elicitation', async () => {
            ...
            expect(result.content[0].text).toContain('⚠️ User cancelled the URL elicitation');
        });
    ...
    describe('get-roots-list', () => {
        ...
        it('should register when client supports roots', () => {
            ...
            registerGetRootsListTool(mockServer);
            expect(mockServer.registerTool).toHaveBeenCalledWith('get-roots-list', expect.objectContaining({
    ...
    describe('simulate-research-query', () => {
        function createMockServerWithTasks() {
            ...
            const mockServer = {
                experimental: {
                    tasks: {
                        registerToolTask: vi.fn((_name, _config, handler) => {
        ...
        function createMockTaskStore(taskId) {
            return {
        ...
        it('should pass relatedTask to sendRequest when elicitation is triggered', async () => {
            ...
            await taskHandlers.createTask({ topic: 'python', ambiguous: true }, { taskStore: mockTaskStore, sendRequest: mockSendRequest });
            await vi.runAllTimersAsync();
            vi.useRealTimers();
            expect(mockSendRequest).toHaveBeenCalledWith(expect.objectContaining({ method: 'elicitation/create' }), expect.anything(), expect.objectContaining({ relatedTask: { taskId: 'task-abc' } }));
        });
        it('should complete without elicitation for non-ambiguous query', async () => {
            ...
            await taskHandlers.createTask({ topic: 'python', ambiguous: false }, { taskStore: mockTaskStore, sendRequest: mockSendRequest });
            await vi.runAllTimersAsync();
            vi.useRealTimers();
            expect(mockSendRequest).not.toHaveBeenCalled();
            expect(mockTaskStore.storeTaskResult).toHaveBeenCalledWith('task-def', 'completed', expect.anything());
        });
    });
    describe('gzip-file-as-resource', () => {
        it('should compress data URI and return resource link', async () => {
            ...
            // Get the handler
            let handler = null;
            mockServer.registerTool.mockImplementation((name, config, h) => {
                handler = h;
            });
            ...
            const result = await handler({ name: 'test.txt.gz', data: dataUri, outputType: 'resourceLink' });
        ...
        it('should return resource directly when outputType is resource', async () => {
            ...
            const dataUri = `data:text/plain;base64,${Buffer.from(testContent).toString('base64')}`;
            const result = await handler({ name: 'output.gz', data: dataUri, outputType: 'resource' });
        ...
        it('should reject unsupported URL protocols', async () => {
            ...
            registerGZipFileAsResourceTool(mockServer);
            await expect(handler({ name: 'test.gz', data: 'ftp://example.com/file.txt', outputType: 'resource' })).rejects.toThrow('Unsupported URL protocol');
        });
    });
});
